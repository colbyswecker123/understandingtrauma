...
await env.DB.prepare("UPDATE products SET name = ?, description = ?, price_cents = ?, image_url = ?, checkout_url = ?, status = ?, sort_order = ?, updated_at = datetime('now') WHERE id = ?").bind(product.name,product.description,product.price_cents,product.image_url,product.checkout_url,product.status,product.sort_order,id).run();
return json({ok:true});
}catch(error){
